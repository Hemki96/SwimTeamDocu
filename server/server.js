...
function ensureDataDirectories(dataDir, { seed = true } = {}) {
  fs.mkdirSync(dataDir, { recursive: true });
  if (seed) {
    if (!fs.existsSync(path.join(dataDir, 'teams.json'))) {
      writeJSONAtomic(path.join(dataDir, 'teams.json'), [{ id: 'TEAM-A', name: '1. Mannschaft', clubId: 'CLUB-1', coachUserIds: ['U-1'] }]);
    }
    if (!fs.existsSync(path.join(dataDir, 'athletes.json'))) {
      writeJSONAtomic(path.join(dataDir, 'athletes.json'), [{ id: 'A-101', name: 'Lias Kühne', birthYear: 2010, gender: 'm', teams: ['TEAM-A'], consents: { media: true, analytics: true } }]);
    }
  }
  fs.mkdirSync(path.join(dataDir, 'sessions'), { recursive: true });
  fs.mkdirSync(path.join(dataDir, 'attendance'), { recursive: true });
}

function createApp({ dataDir = DEFAULT_DATA_DIR, publicDir = DEFAULT_PUBLIC_DIR, seed = true } = {}) {
  ensureDataDirectories(dataDir, { seed });

  const server = http.createServer(async (req, res) => {
    // Static
    if (req.method === 'GET' && serveStatic(req, res, publicDir) !== false) return;
    ...
    if (req.method === 'GET' && pathname === '/api/athletes') {
      const list = readJSON(path.join(dataDir, 'athletes.json'), []);
    ...
    if (req.method === 'GET' && pathname.startsWith('/api/analytics/team/')) {
      ...
      const files = fs.readdirSync(path.join(dataDir, 'sessions')).filter(f => f.endsWith('.json'));
      const sessions = files.map(f => readJSON(path.join(dataDir, 'sessions', f), null)).filter(Boolean).filter(s=>s.teamId===teamId);
      ...
      const attendanceFiles = fs.readdirSync(path.join(dataDir, 'attendance'));
      let present = 0, total = 0;
      for(const fpath of attendanceFiles){
        const sid = path.basename(fpath, '.jsonl');
        const sess = sessions.find(s=>s.id===sid);
        if(!sess) continue;
        const content = fs.readFileSync(path.join(dataDir,'attendance',fpath),'utf-8').trim();
        if (!content) continue;
    ...
    if (req.method === 'GET' && pathname === '/api/rules') {
      const file = path.join(dataDir, 'rules.yaml');
  ...
  // API: list sessions (very basic)
    if (req.method === 'GET' && pathname === '/api/sessions') {
      ...
      const files = fs.readdirSync(path.join(dataDir, 'sessions')).filter(f => f.endsWith('.json'));
      const sessions = files.map(f => readJSON(path.join(dataDir, 'sessions', f), null)).filter(Boolean);
...
// API: get session by id
    if (req.method === 'GET' && pathname.startsWith('/api/sessions/') && pathname.split('/').length === 4) {
      ...
      const file = path.join(dataDir, 'sessions', `${id}.json`);
  ...
  // API: attendance (append JSONL per session)
    if (req.method === 'POST' && pathname.startsWith('/api/sessions/') && pathname.endsWith('/attendance')) {
      const id = pathname.split('/')[3];
      const file = path.join(dataDir, 'attendance', `${id}.jsonl`);
      try {
